...
def point_cloud_sampling(point_cloud:np.ndarray, num_points:int, method:str='fps', device='cpu'):
    """
    ...
    if method == 'uniform':
        ...
        # print(f"in shape: {point_cloud.shape[0]}.  out shape: {num_points}")
    ...
    def step(self, action):
        ...
        if 'adroit' in self.env_name: # adroit uses a namedtuple for obs
            ...
            timestep = ExtendedTimeStepAdroit(step_type=timestep.step_type,
                                         ...
                                         observation_full_state=timestep.observation_full_state,
